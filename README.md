# Self Spinning Tricopter Project
## Brief Introduction of Self Spinning Tricopter ##

Self Spinning Tricopter is a novel tricopter with a unique flight mode, namely self spinning mode. In the project, a spinning tricopter with a highly simplified structure is designed. During the flight of the spinning tricopter, the torque generated by the rotation of the rotor in the same direction drives the fuselage to spin in the opposite direction, which generates additional lift at the wings while achieving better stability.

The flying test of the self-spinning tricopter is demonstrated in the short video in the following link:
- <.com>

## The main contribution of the modified flight control code include: ##

- Coordinate Transformation : Attitude control of the orignal APM source code is based on Euler Angle, which is not applicable when the tricopter is self-spinning. We propose a coordinate which is able to provide stable control volumes.

- PID Control Scheme Adaptation : In order to adapt to the new coordinate, we modify the control scheme under the PID control strategy framework of the original APM source code.

- Periodic Control Strategy : Since that the position of rotor in space is constantly changing, the thrust volume provided by the rotor is a function to both time and space. We utilize this function to the control scheme to reflect the self-spinning status and calculate the thrust volume of each motor according to current status.

- Paramter Monitor : Some important flight paramters are newly proposed, and therefore not included in the record of flight journal and not convenient for real-time monitor during flight test. We modify the source code and allow the paramters to be dispalyed real time in the ground control station Mission Planner.

## Modification to the source code are mainly in: ##
- ArduCopter/ArduCopter.cpp
- libraries/AC_AttitutdeControl/AC_AttitudeControl.cpp
- libraries/AC_AttitutdeControl/AC_AttitudeControl_Multi.cpp
- libraries/GCS_MAVLink/GCS.cpp

# The original source code of this project is based on the open source APM Copter-3.6
